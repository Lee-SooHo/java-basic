...
치타의 속도(m/s, 정수)와, 사냥감의 거리(m, 정수)와 속도(m/s, 정수)가 주어졌을 때 치타가 사냥에 성공하면 몇초가 걸려 잡았는지, 실패하면 "꼬르륵"을 출력하여라.
걸린 시간은 올림하여 정수로 표현한다. 단 치타와 사냥감은 같은 직선상에 움직이며, 치타와 사냥감 둘다 속도가 변하지 않는다.
 */
